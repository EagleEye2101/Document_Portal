...
class ConversationalRAG:
    def __init__(self,session_id:str,retriever):
        try:
            ...
            self.log.info("Created history-awae retriever", session_id=session_id)
            ...
            raise DocumentPortalException("Failed to initialize CoversationalRAG",sys)
    def _load_llm(self):
        try:
    ...
    def load_retriever_from_fiass(self,index_path:str):
         try:
    ...
    def invoke(self,user_input:str)->str:
        try:
            ...
            if not answer:
                self.log.warning("Empty answer recieved",session_id=self.session_id)
            self.log.info("Chain invoked succesfully",session_id=self.session_id,user_input=user_input,answer_preview=answer[:150])
            return answer
        except Exception as e:
